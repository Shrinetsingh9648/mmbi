...
── Architectural decision re: HFA vs. the MicroLSTM pathway ────────────────
(Implementation spec Phase 13.) Four options were considered for this
network: (1) keep as a standalone CNN visual feature extractor whose
per-frame embedding feeds into MicroLSTM's fused feature vector, (2) keep
running as a fully independent model (current choice), (3) fuse its
per-frame CNN embedding into temporal/buffer.py's feature vector so
MicroLSTM consumes it directly, or (4) disable it during training.

DECISION: keep it as an INDEPENDENT model (option 2), not fused into the
BiLSTM sequence (option 3), for this pass. Reasoning:
  - Option 3 requires running this CNN on every buffered frame in
    real time so its embedding can be pushed into TemporalBuffer
    alongside the AU/gaze/head/flow features every frame. On CPU-only
    inference hardware (the deployment target per Phase 16) that is a
    meaningfully heavier per-frame cost than the current geometric+flow
    feature path, and doing it without profiling real hardware would be
    guessing, not engineering.
  - The audit's Option C recommendation is about being *able* to fuse a
    CNN visual stream when there's headroom for it, not a mandate to do
    so immediately at the cost of CPU-friendliness (Phase 16 explicitly
    asks to avoid this).
  - This network's AU head remains architecturally present but
    UNSUPERVISED (see hfa_trainer.py: au_weight=0.0) and it has no phase
    or intensity head at all — adding those without real AU-coded data
    to train them would just create more untrained outputs, which
    Implementation Rule #3 (do not fake training) prohibits doing
    casually.
  - This is a genuinely incremental, reversible choice: if you later
    profile real hardware and find CPU headroom, wiring this CNN's
    embedding into temporal/buffer.py's FEATURE_KEYS (as a 6th block of
    features alongside AU/gaze/head/flow) is a well-scoped follow-up,
    not a rewrite. It is intentionally NOT done here — see the
    implementation report's "BLOCKED" list.

What this network is used for right now: an auxiliary, independently
inferred emotion signal (dominant_emotion() in fusion/fusion_layer.py
already picks between MicroLSTM's and this network's emotion output by
confidence) and an attention-heatmap visualization overlay. It is not
part of the ME spotting/phase/intensity pipeline.
"""
...
# NOTE: "133 ms at 120 FPS" assumes a camera speed most webcams cannot
# sustain (see capture/camera.py.measure_actual_fps() / technical audit
# §1/§4). Use the measured FPS to know what T_FRAMES actually represents
# in wall-clock time on your hardware.
